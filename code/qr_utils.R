quantreg_trendfilter = function(y, q=0.5, k=3L, lambda = 1, rel_tol = 1e-4, max_it = 100, method = "genlasso", penalty = "l1", pen.par = NULL, objtol = 1e-6, max.iter = 1e4) {
  ...
  require(genlasso)
	...
	while(!converged && it_counter <= max_it) {
		...
		if (method == "genlasso") {
		  ...
		  tfk = tfprox(z = z, w = weights, D = D, penalty = penalty, pen.par = pen.par, lambda = lambda, max.iter = max.iter, objtol = objtol)
		}
		beta_hat = tfk$beta
