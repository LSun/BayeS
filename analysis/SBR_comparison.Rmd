...
cv.sbr = function (A, z, lambda, nfold = 10) {
  ...
  mse_cv = c()
  for (i in 1:nfold) {
    betahat = sbr(A = A[-fold[[i]], ], z = z[-fold[[i]]], lambda)$x
...
The design matrix $X$ is column-wise normalized such tht for each column $X_j$, $\bar X_j = 0$, $\|X_j\|_2 = 1$.  The coefficients $\beta$ is generated such that $\eta = 90\%$ of them are zero, and $1 - \eta = 10\%$ of them are uniformly simulated from $[-a, a]$.  Thus the variance of $\beta = (1-\eta)a^2 / 3$.  Under this setting, the signal-to-noise ratio (SNR, in db) is defined as $10\log_{10}\left(\frac{p\text{var}\beta}{n\text{var}e}\right) = 10\log_{10}\frac{(1-\eta)pa^2}{3n\sigma^2}$.
...
beta = rep(0, p)
beta[sample(p, k)] = runif(k, -a_beta, a_beta)
# beta[sample(p, k)] = rnorm(k, sd = sd_beta)
sd_noise = sqrt(p * sd_beta^2 * 10^(-SNR / 10) / n)
