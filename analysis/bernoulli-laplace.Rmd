...
plot(z, z, type = "n", ylim = range(z), xlab = "Observation y", ylab = expression(paste("Posterior Mean E[", theta,"|y]")), main = bquote(paste(hat(theta)(y), ", Spike-and-Slab Lasso with", ~p == .(pi), ", ", ~sigma[0] == .(s))))
...
col = rainbow(length(sd))
plot(u, u, type = "n", ylim = c(0, ymax), xlab = expression(theta), ylab = expression(phi(theta)), main = bquote(paste("Penalty ", phi, ", Spike-and-Slab Lasso with", ~p == .(pi), ", ", ~sigma[0] == .(s))))
