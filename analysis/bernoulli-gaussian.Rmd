...
plot(z, pm4, type = "n", xlab = "Observation y", ylab = expression(paste("Posterior Mean E[", theta,"|y]")),
     col = "blue", main = bquote(paste(hat(theta)(y), ", Spike-and-Slab Gaussian with", ~p == .(pi), ",", ~sigma[0] == .(s)))
     )
...
abline(0, 1, lty = 3)
abline(h = 0, lty = 3)
legend("topleft", col = rainbow(5), lty = 1, c(expression(sigma == 1), expression(sigma == 2), expression(sigma == 5), expression(sigma == 10), expression(sigma == 100)), ncol = 2)
```
...
plot(u, phi(u, s, sigma = 1, pi), type = "n", ylim = c(0, ymax),
     xlab = expression(theta), ylab = expression(phi(theta)),
     main = bquote(paste("Penalty ", phi, ", Spike-and-Slab Gaussian with", ~p == .(pi), ",", ~sigma[0] == .(s)))
     )
